...
printf '%s\n' '-----BEGIN OPENSSH PRIVATE KEY-----' existing-sensitive-material >"$HOME/.ssh/wintix_github_ed25519"
...
grep -F -- '--decrypt' "$SOPS_LOG" >/dev/null; grep -q '^sops:' "$WINTIX_PATH/secrets/github-ssh-key.yaml"
